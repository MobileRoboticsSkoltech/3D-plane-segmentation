...
void DDPFF::drawSeedPoints() const
{
#if DEBUG_DDPFF==1
    static Vec3 from, to;

    for (size_t i = 1; i < seedIndices_.size(); i++) {
        if (unmergedPlanes_[i].count < config.minPlaneSize) {
            continue;
        }

        const size_t& startIndex = seedIndices_[i];
        const Seed& startPoint = seedPoints_[i];

        const size_t r = startIndex / width_, c = startIndex % width_;
        const size_t radius = seedRadii_[i];

        const auto& north = (*pBuf_ptr)[width_ * (r - radius) + c];
        const auto& south = (*pBuf_ptr)[width_ * (r + radius) + c];
        const auto& east = (*pBuf_ptr)[width_ * r + (c + radius)];
        const auto& west = (*pBuf_ptr)[width_ * r + (c - radius)];

        if (notValid(north) || notValid(south)|| notValid(east) || notValid(west)) {
            continue;
        }

        const QColor& color = unmergedPlanes_[i].color;
        glColor4f(color.redF(), color.greenF(), color.blueF(), 0.5);

        from = (*T_ptr) * startPoint.point;
        to = (*T_ptr) * (startPoint.point + 0.05 * startPoint.normal);

        QGLViewer::drawArrow(qglviewer::Vec(from), qglviewer::Vec(to), 0.003);

        glPushMatrix();
        glMultMatrixd(T_ptr->data());

        glPointSize(10);
        glBegin(GL_POINTS);
        // Draw the neighbours as points used to calculate the seed
        glVertex3dv(north.data());
        glVertex3dv(south.data());
        glVertex3dv(east.data());
        glVertex3dv(west.data());
        glEnd();

        // Draw lines connecting to the neighbours
        glLineWidth(1);
        glBegin(GL_LINES);
        glVertex3dv(startPoint.point.data());
        glVertex3dv(north.data());
        glEnd();
...
void DDPFF::drawSeedPoints(QPainter *painter) const
{
#if DEBUG_DDPFF==1
    painter->save();
    ...
    for (size_t i = 1; i < seedIndices_.size(); i++) {
        ...
        const size_t& startIndex = seedIndices_[i];
        const size_t c = sampleFactor_ * (startIndex % width_);
        const size_t r = sampleFactor_ * (startIndex / width_);
        const size_t radius = seedRadii_[i];
        ...
        painter->drawEllipse(c, r, 2, 2);

        // Draw the neighbours used to calculate the seed
        painter->drawEllipse(c, r - radius, 1, 1); // north
        painter->drawEllipse(c, r + radius, 1, 1); // south
        painter->drawEllipse(c + radius, r, 1, 1); // east
        painter->drawEllipse(c - radius, r, 1, 1); // west

        // Draw lines to neighbours
        painter->drawLine(c, r, c, r - radius);
        painter->drawLine(c, r, c, r + radius);
        painter->drawLine(c, r, c + radius, r);
        painter->drawLine(c, r, c - radius, r);
    }
...
void DDPFF::drawUnmergedPlanes() const
{
    static Vec3 to, from;
    ...
    for (size_t r = 0; r < IMAGE_HEIGHT; r++) {
        for (size_t c = 0; c < IMAGE_WIDTH; c++) {
            const size_t& originalIndex = IMAGE_WIDTH * r + c;
            const size_t& bufferIndex = width_ * (r / sampleFactor_) + (c / sampleFactor_);

            const unsigned_t label = unmergedLabels_[bufferIndex];

            if (label == BACKGROUND) {
                // This point does not belong to any plane.
                continue;
            }

            const QColor& color = unmergedPlanes_[label].color;
            const Vec3& point = (*pBuf_ptr)[originalIndex];
    ...
    if (command.showPlaneNormals)
    {
        for (size_t i = 0; i < unmergedPlanes_.size(); i++) {
            const auto& pn = unmergedPlanes_[i];


            if (pn.count < config.minPlaneSize) {
                continue;
            }

            const Vec3& unmergedNormal = pn.normal;
            const Vec3& unmergedCentroid = pn.point;

            if (notValid(unmergedNormal) || notValid(unmergedCentroid)) {
                continue;
            }

            from = (*T_ptr) * unmergedCentroid;
            to = (*T_ptr) * (unmergedCentroid + 0.1 * unmergedNormal);

            const QColor& color = unmergedPlanes_[i].color;
            glColor4ub(color.red(), color.green(), color.blue(), 255);
            QGLViewer::drawArrow(qglviewer::Vec(from), qglviewer::Vec(to), 0.005);
        }
    }
}

void DDPFF::drawUnmergedPlanes(QPainter *painter) const
{
    painter->save();
    ...
    for (size_t r = 0; r < IMAGE_HEIGHT; r++) {
        for (size_t c = 0; c < IMAGE_WIDTH; c++) {
            ...
            const QColor& color = unmergedPlanes_[label].color;
            if (unmergedPlanes_[label].count < config.minPlaneSize) {
                continue;
            }
    ...
    if (command.showText)
    {
        ...
        QFont labelFont;
        labelFont.setPointSize(6); labelFont.setBold(true);
        for (size_t i = 0; i < unmergedPlanes_.size(); i++) {
            if (unmergedPlanes_[i].count < config.minPlaneSize) {
                continue;
            }
            const Vec2i& pixel = unmergedPlanes_[i].pixel;
            const QColor& color = unmergedPlanes_[i].color;
            painter->setOpacity(1.0);
            painter->setPen(Qt::white);
            painter->setFont(labelFont);
            painter->drawText(pixel.x(), pixel.y(), QString::number(i));
            underline.setColor(color);
            painter->setPen(underline);
            painter->drawLine(pixel.x(), pixel.y() + 2, pixel.x() + 15, pixel.y() + 2);
        }
    }
...
void DDPFF::drawMergedPlanes() const
{
    glPushMatrix();
    glMultMatrixd(T_ptr->data());
    glPointSize(3);
    ...
    for (size_t r = 0; r < IMAGE_HEIGHT; r++) {
        for (size_t c = 0; c < IMAGE_WIDTH; c++) {
            ...
            const unsigned_t label = mergedLabels_[bufferIndex];
            ...
            const planeSegParamsPtr_t& planeParam = mergedPlanes_[label];

            if (!planeParam) {
                continue;
            }

            if (planeParam->count < config.minPlaneSize) {
                // Don't show planes with inlier count less than the threshold
                continue;
            }

            const QColor& color = planeParam->color;
            const Vec3& point = (*pBuf_ptr)[originalIndex];
            glColor4ub(color.red(), color.green(), color.blue(), 128);
    ...
    if (command.showPlaneNormals) {
        ...
        for (auto& planeParam: mergedPlanes_) {
            ...
            const QColor& c = planeParam->color;
            glColor4ub(c.red(), c.green(), c.blue(), 255);

            from.fill(0); to.fill(0);

            from = (*T_ptr) * planeParam->point;
            to = (*T_ptr) * (planeParam->point + 0.1 * planeParam->normal);
...
void DDPFF::drawMergedPlanes(QPainter *painter) const
{
    painter->save();
    ...
    if (command.showText)
    {
        ...
        for (const auto& planeParam: mergedPlanes_) {
            if (!planeParam) {
                continue;
            }

            if (planeParam->count < 3) {
                // Don't show degenerate planes
                continue;
            }
            ...
            const Vec2i& pixel = planeParam->pixel;
            const QColor &color = planeParam->color;
            painter->setOpacity(1.0);
            painter->setPen(Qt::white);
            painter->setFont(labelFont);
            painter->drawText(pixel.x(), pixel.y(), QString::number(planeParam->planeId));
            underline.setColor(color);
            painter->setPen(underline);
            painter->drawLine(pixel.x(), pixel.y() + 2, pixel.x() + 15, pixel.y() + 2);
        }
...
void DDPFF::drawUnmarkedSegments(QPainter *painter) const
{
#if DEBUG_DDPFF==1
    painter->save();
    ...
    for (const auto &s: unmarkedSegments_) {
        if (s.dir == 2) {
            pen.setColor(Qt::red);
        } else if (s.dir == 3) {
        ...
        painter->setPen(pen);
        for (size_t x = s.lx; x <= s.rx; x++) {
            painter->drawPoint(x, s.y);
        }
    }
...
void DDPFF::drawGapLabels() const
{
#if DEBUG_DDPFF==1
    glPushMatrix();
    glMultMatrixd(T_ptr->data());
    ...
    for (size_t r = 0; r < IMAGE_HEIGHT; r++) {
        for (size_t c = 0; c < IMAGE_WIDTH; c++) {
            ...
            const unsigned_t label = gapLabels_[bufferIndex];
            ...
            const planeSegParamsPtr_t& planeParam = gapPlanes_[label];
    ...
    if (command.showPlaneNormals) {
        ...
        for (auto& planeParam: gapPlanes_) {
            ...
            from = (*T_ptr) * planeParam->point;
            to = (*T_ptr) * (planeParam->point + 0.05 * planeParam->normal);
...
void DDPFF::drawGapLabels(QPainter *painter) const
{
#if DEBUG_DDPFF==1
    painter->save();
    ...
    if (command.showText)
    {
        ...
        QFont labelFont;
        labelFont.setPointSize(6);
        for (const auto& planeParam: gapPlanes_) {
            if (!planeParam) {
                continue;
            }
...
void DDPFF::drawUnorderedPlanes() const
{
#if DEBUG_DDPFF==1
    glPushMatrix();
    glMultMatrixd((*T_ptr).data());
    ...
    for (const auto &p: unorderedPlanes_) {
        const auto &color = p.color;

        for (auto idx: p.inliers) {
            const auto &point = (*pBuf_ptr)[idx];
            glColor4f(color.redF(), color.greenF(), color.blueF(), 0.5);
            glVertex3d(point.x(), point.y(), point.z());
        }
    }
...
void DDPFF::drawUnorderedPlanes(QPainter *painter) const
{
#if DEBUG_DDPFF==1
    painter->save();
    painter->setOpacity(0.5);
    ...
    for (const auto &p: unorderedPlanes_) {
        ...
        for (auto idx: p.inliers) {
            const auto r = (idx / IMAGE_WIDTH) / sampleFactor_;
            const auto c = (idx % IMAGE_WIDTH) / sampleFactor_;
            painter->setPen(color);
            painter->drawPoint(c,r);
        }
    }
    ...
    if (command.showText)
    {
        ...
        QFont labelFont;
        labelFont.setPointSize(6);
        for (const auto& planeParam: unorderedPlanes_) {
            if (planeParam.count < 3) {
                // Don't show degenerate planes
                continue;
            }
...
void DDPFF::compute()
{
    ...
    qDebug() << "DDPFF time (ms):" << sw.elapsedTimeMs();

}

void DDPFF::draw() const
{
    if (!command.showViz)
    {
        return;
    ...
    if (command.showUnmergedPlanes) {
        drawUnmergedPlanes();
    }
    if (command.showMergedPlanes) {
        drawMergedPlanes();
    }

    if (command.showStartPoints) {
        drawSeedPoints();
    }
    ...
    if (command.experimentalFeatureOn) {
        drawUnorderedPlanes();
    }
}

void DDPFF::draw(QPainter *painter) const
{
    if (!command.showViz)
    {
        return;
    ...
    if (command.experimentalFeatureOn) {
        drawUnorderedPlanes(painter);
    }

    if (command.showUnmergedPlanes) {
        drawUnmergedPlanes(painter);
    }
    if (command.showMergedPlanes) {
        drawMergedPlanes(painter);
    }

    if (command.showStartPoints) {
        drawSeedPoints(painter);
    }

    if (command.showGaps) {
        drawGapLabels(painter);
    }
}
...
{
    dbg << "pid| " << o.planeId << "| p:" << o.point << ", n:" << o.normal << ", depth:" << o.maxDepth << ", count:" << o.count;
    dbg << "\n" << "nbrs:";
