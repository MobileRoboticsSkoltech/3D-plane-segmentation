...
# Install kdevelop as a secondary IDE
RUN apt-get update && apt-get full-upgrade -y && apt-get install -y --no-install-recommends \
    wget
RUN wget -O /KDevelop.AppImage https://download.kde.org/stable/kdevelop/5.6.0/bin/linux/KDevelop-5.6.0-x86_64.AppImage && chmod +x /KDevelop.AppImage
